...
vector<float> runfit(string input, string histname){
	...
	TF1* func = new TF1("func",fit_Exp_c,0,200,3); // bound min and max, free params
	func->SetParLimits(2,0,5);
	
	TF1* func_for_plot = new TF1("func4plot",fit_Exp_c,0,1000,3);
	...
	*/
	...
	cout << "chi2 = " << chi2 << endl;
	cout << "chi2/ = " << chi2 / (float) (h->GetNbinsX() - 3) << endl;
	for (auto x : params){
		cout << x << " : error : " << errors[count] << endl;
		func_for_plot->FixParameter(count,x);
		count++;
	}
...
void fit_curve(string input, string output = "fit_result.root", bool isTest = false){
