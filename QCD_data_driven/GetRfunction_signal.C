...
void GetRfunction_signal(string inputfile, string outfile, bool isTest = false){
	...
	map<int , int> pp;
	string paths[6] = {"HLT_PFHT180", "HLT_PFHT250", "HLT_PFHT430", "HLT_PFHT510", "HLT_PFHT590", "HLT_PFHT1050"};
	int HTUseRange[7] = {200, 300, 500, 600, 750, 1200, 100000};
	int trigScale[6] = {1, 1000, 150, 75, 30, 1};
	...
	while (getline(flist,line) ){
		...
		while (data.Next() ){
			...
			h_HT->Fill(offline_HT,thisTrigScale);
			h_MET_[whichHT]->Fill(*METPT,thisTrigScale);
			//if (offline_HT > 200) h_MET_HTcut->Fill(*METPT,thisTrigScale);
			if (mindphi > 0.4) h_met_mindphi_l[whichHT]->Fill(*METPT,thisTrigScale);
			else h_met_mindphi_s[whichHT]->Fill(*METPT,thisTrigScale);
			metpT = -999; // use -999 as default value;
			metpT = *METPT;
			ot.Fill();
		} // end of entries
	}// end of file list
