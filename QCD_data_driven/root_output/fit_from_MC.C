...
void fit_from_MC(string inputfile){
	...
	TFile* fin = new TFile("../../SRcut_test/fit_result.root","READONLY"); // this file name is default
	...
	TF1* func = new TF1("func",fit_Exp_c,0,1000,4); // bound min and max, free params
	cout << "rFactor size: " << rFactor[5].size() << endl;
	for (int i=0; i<rFactor[5].size(); i++){
		func->FixParameter(i,rFactor[5][i]);
	}
	
	fin->Close();
	fin = new TFile("keep_histo_cutFull.root","READONLY");
	TH1F* h = (TH1F*) fin->Get("h_withSR_5");
	h->Fit("func");
